...
const MedicationDetailsSection = ({
  ...
  const [presentationValue, setPresentationValue] = useState(
    donationForm.Presentation || ""
  );
  const [formValue, setFormValue] = useState(donationForm.Form || "");
  const [laboratoryValue, setLaboratoryValue] = useState(
    donationForm.Laboratory || ""
  );
  const [labCountryValue, setLabCountryValue] = useState(
    donationForm.LaboratoryCountry || ""
  );
