...
//   const [selectedDonation, setSelectedDonation] = useState(null); // Remove this state
...
//               onPress={() => {
//                 if (selectedItemIds.length === 0) {
//                   // Remove setSelectedDonation here
//                 } else {
...
//       {/* Remove selectedDonation rendering here */}
...
const Donation = () => {
  ...
  const toggleCheckbox = (item) => {
    const index = selectedItemIds.indexOf(item.DonationId);
    const updatedIds = [...selectedItemIds];
    if (index !== -1) {
      updatedIds.splice(index, 1);
    } else {
      updatedIds.push(item.DonationId);
    }
  ...
  return (
    <View style={styles.mainContainer}>
        ...
        renderItem={({ item }) => {
          ...
          return (
            <TouchableOpacity
              ...
              onPress={() => {
                if (selectedItemIds.length === 0) {
                  setSelectedDonation(item); // Set selected donation for detailed view
                } else {
                  toggleCheckbox(item); // Handle checkbox toggle if items are selected
                }
              }}
              onLongPress={() => {
                if (selectedItemIds.length === 0) {
      ...
      />
      {/* Render DonationDetails */}
      {!selectedItemIds.length && selectedDonation && (
      ...
      >
        <View style={styles.modalContainer}>
          <View style={styles.modalContent}>
            <Text style={styles.modalText}>Are you sure you want to deselect all items?</Text>
            <View style={styles.modalButtons}>
              {/* Directly clear selected items */}
              <Button style={styles.yesBtn} onPress={handleModalConfirm}>Yes</Button>
              <Button style={styles.noBtn} onPress={handleModalCancel}>No</Button>
            </View>
          </View>
        </View>
      </Modal>
